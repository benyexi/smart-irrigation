...
- `mqttClient.ts` 现已改为运行时按需注入本地静态资源 `public/mqtt.min.js`，MQTT 浏览器运行库不再进入应用 bundle，`vendor-mqtt` 构建块已消失。
- 图表层也已改为“轻量 React 包装层 + `public/echarts.min.js` 按需注入”，`vendor-echarts` 构建块已消失。
